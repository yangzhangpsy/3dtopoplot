% topoplotNew() - plot a topographic map of a scalp data field in a 2-D circular view
...
% Usage:
%        >>  topoplotNew(datavector, EEG.chanlocs);   % plot a map using an EEG chanlocs structure
%        >>  topoplotNew(datavector, 'my_chan.locs'); % read a channel locations file and plot a map
%        >>  topoplotNew('example');                  % give an example of an electrode location file
%        >>  [h grid_or_val plotrad_or_grid, xmesh, ymesh]= ...
%                           topoplotNew(datavector, chan_locs, 'Input1','Value1', ...);
% Required Inputs:
...
%                       Or a vector defined the values of the contourlines
...
%  Rev. by Yang Zhang Thu Apr 19 23:07:34 2018
% Soochow University, China
% added support to plot fake 3d topoplot

function [handle,Zi,grid,Xi,Yi,contourLineVals,hp2b] = topoplotNew(Values,loc_file,varargin)

%

is3Dmode = true; % added by yang 2016-09-24 13:34:37

cFolder = fileparts(mfilename('fullpath'));
%%%%%%%%%%%%%%%%%%%%%%%% Set defaults %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% icadefs                 % read defaults MAXTOPOPLOTCHANS and DEFAULT_ELOC and BACKCOLOR
if ~exist('BACKCOLOR','var')  % if icadefs.m does not define BACKCOLOR
...
GRID_SCALE         = 129;        % plot map on a 67X67 grid
CIRCGRID           = 361;       % number of angles to use in drawing circles
AXHEADFAC          = 1.3;        % head to axes scaling factor
CONTOURNUM         = 6;         % number of contour levels to plot
contourLineWidth   = 1;   % the width of the contour lines
HEADCONTOURDOWNWIDTH  = 1;   % the width of the down side head contour lines % added by yang 2016-09-26 2:46:26
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%----- added by yang 2018/8/29 9:33:26--/
if nargout>6
	hp2b = []; % for output argout
end
%---------------------------------------\
%
%%%%%%%%%%%%%%%%%%%%%%% Handle arguments %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if isstruct(Values) | ~isstruct(loc_file)
    fieldtrip = 1;
end; %#ok<*OR2>

if ischar(loc_file)
    if exist(loc_file) ~= 2 %#ok<*EXIST>
        fieldtrip = 1;
    end;
end;
...
if nargs == 1
    if ischar(Values)
        if any(strcmpi(Values,{'example','demo'}))
...
isInputContourVals = false; % added by yang 2015/8/13 20:41:34
...
if nargs > 2
    ...
    for i = 1:2:length(varargin)
        ...
        switch Param
            ...
            case 'nosedir'
                NOSEDIR = Value;
                if isempty(strcmpi(NOSEDIR, { '+x', '-x', '+y', '-y' }))
            ...
            case 'headContourDownWidth'
                HEADCONTOURDOWNWIDTH = Value;
            case 'electrodes'
            ...
            case 'dipscale'
                DIPSCALE = Value;
            case 'contourvals'
                isInputContourVals = true; % added by yang 2015/8/13 20:41:00
                ContourVals        = Value;
            case 'pmask'
                ContourVals = Value;
                PMASKFLAG   = 1;
            ...
            case 'shading'
                ...
                if strcmpi(SHADING,'interp') && isempty(warningInterp)
                    warning('Using interpolated shading in scalp topographies prevent to export them as vectorized figures'); %#ok<*WNTAG>
                    warningInterp = 1;
                end;
            case 'noplot'
                noplot = Value;
            ...
            case {'whitebk','whiteback','forprint'}
                whitebk = Value;
            case 'viewpoint' % added by yang 2016-09-25 1:14:09
                viewpoint = lower(Value);
            otherwise
...
%---- added by yang 2016-09-25 1:14:46 ------/
% check default parameters for viewpoint
...
switch viewpoint
    ...
    case {'left','right','back','front'}
        % fprintf('in back view point: INTERPLIMITS should be set to ''electrodes''\n\n');
        fprintf('      <<<<<< prameter check >>>>>>\n');
        whitebk = 'on';
        fprintf(['In ',viewpoint,' view point: whitebk should be set to ''on''\n']);
        
        if isempty(plotrad)|plotrad == 0.5
            plotrad = 0.500;
            fprintf(['In ',viewpoint,' view point: plotrad should be set to 0.5\n']);
        end
        
        GRID_SCALE = 513;        % plot map on a 256X256 grid
        fprintf(['In ',viewpoint,' view point: GRID_SCALE should be set to 513\n']);
    case 'top'
        
    otherwise
        error('viewpoint should be of {''back'',''top'',''left'',''right''}');
end
...
if ~strcmpi(STYLE,'grid')                     % if not plot grid only
    ...
    labels      = strvcat(labels); %#ok<*REMFF1> % make a label string matrix
    ...
    %
    if isempty(plotrad) && isfield(tmpeloc, 'plotrad'), %#ok<*NOCOL>
        plotrad = tmpeloc(1).plotrad;
        if ischar(plotrad)                        % plotrad shouldn't be a string
            plotrad = str2num(plotrad) ;          %#ok<*ST2NM> % just checking
        end
    ...
    if ~isempty(shrinkfactor) | isfield(tmpeloc, 'shrink'),
        ...
        if ischar(shrinkfactor)
            ...
            if strcmpi(VERBOSE,'on')
                fprintf('    %g%% shrink  applied.'); %#ok<CTPCT>
                if abs(headrad-rmax) > 1e-2
    ...
    allx      = x;
    ally      = y;
    intchans; %#ok<*VUNUS> % interpolate using only the 'intchans' channels
    ...
    if ~isempty(EMARKER2CHANS)
        if strcmpi(STYLE,'blank')
            ...
            [tmp1,mark1chans,tmp2] = setxor(pltchans,EMARKER2CHANS); %#ok<*ASGLU>
...
if ~strcmpi(STYLE,'blank') % if draw interpolated scalp map
    if ~strcmpi(STYLE,'grid') %  not a rectangular channel grid
        ...
        try
            [Xi,Yi,Zi]  = griddata(inty,intx,double(intValues),yi',xi,'v4'); %#ok<*GRIDD> % interpolate data
        ...
        switch viewpoint
            ...
            case 'back'
                ...
                headData    = load(fullfile(cFolder,'headContoursBKview.mat'));
                movePixes   = [0 0];
                scaleRation = (headData.brainWidth)/(plotrad*2);
                % disp(plotrad*2);
                
                headData.headContourLine1 = (headData.headContourLine1+repmat(movePixes,size(headData.headContourLine1,1),1)) /scaleRation;
                headData.headContourLine2 = (headData.headContourLine2+repmat(movePixes,size(headData.headContourLine2,1),1)) /scaleRation;
                
                headData.leftEar1         = (headData.leftEar1+repmat(movePixes,size(headData.leftEar1,1),1)) /scaleRation;
                headData.leftEar2         = (headData.leftEar2+repmat(movePixes,size(headData.leftEar2,1),1)) /scaleRation;
                
                headData.rightEar1        = (headData.rightEar1+repmat(movePixes,size(headData.rightEar1,1),1)) /scaleRation;
                headData.rightEar2        = (headData.rightEar2+repmat(movePixes,size(headData.rightEar2,1),1)) /scaleRation;
                
                headData.headMaskPolygon  = (headData.headMaskPolygon+repmat(movePixes,size(headData.headMaskPolygon,1),1)) /scaleRation;
                
                
                mask = inpolygon(Xi,Yi,headData.headMaskPolygon(:,1),headData.headMaskPolygon(:,2));
                
                % mask = true(size(Xi));
            case {'left','right'}
                %------ scale the data to match the current EEG MAP size -----/
                headData    = load(fullfile(cFolder,['headContours',upper(viewpoint(1)),viewpoint(2:end),'view.mat']));
                movePixes   = [0 0];
                scaleRation = (headData.brainWidth)/(plotrad*2);
                
                headData.headContour1 = (headData.headContour1+repmat(movePixes,size(headData.headContour1,1),1)) /scaleRation;
                headData.headContour2 = (headData.headContour2+repmat(movePixes,size(headData.headContour2,1),1)) /scaleRation;
                headData.headContour3 = (headData.headContour3+repmat(movePixes,size(headData.headContour3,1),1)) /scaleRation;
                
                headData.eyebrow1 = (headData.eyebrow1+repmat(movePixes,size(headData.eyebrow1,1),1)) /scaleRation;
                
                headData.eyeLine1 = (headData.eyeLine1+repmat(movePixes,size(headData.eyeLine1,1),1)) /scaleRation;
                headData.eyeLine2 = (headData.eyeLine2+repmat(movePixes,size(headData.eyeLine2,1),1)) /scaleRation;
                
                headData.headDownLine = (headData.headDownLine+repmat(movePixes,size(headData.headDownLine,1),1)) /scaleRation;
            ...
            case 'front'
                headData    = load(fullfile(cFolder,'headContoursFrontview.mat'));
                movePixes   = [0 0];
                scaleRation = (headData.brainWidth)/(plotrad*2);
                ...
                headData.leftEye1 = (headData.leftEye1+repmat(movePixes,size(headData.leftEye1,1),1)) /scaleRation;
                headData.leftEye2 = (headData.leftEye2+repmat(movePixes,size(headData.leftEye2,1),1)) /scaleRation;
                
                headData.leftEyebrow1 = (headData.leftEyebrow1+repmat(movePixes,size(headData.leftEyebrow1,1),1)) /scaleRation;
                headData.leftEyebrow2 = (headData.leftEyebrow2+repmat(movePixes,size(headData.leftEyebrow2,1),1)) /scaleRation;
                
                
                headData.rightEar1 = (headData.rightEar1+repmat(movePixes,size(headData.rightEar1,1),1)) /scaleRation;
                headData.rightEar2 = (headData.rightEar2+repmat(movePixes,size(headData.rightEar2,1),1)) /scaleRation;
                
                headData.rightEye1 = (headData.rightEye1+repmat(movePixes,size(headData.rightEye1,1),1)) /scaleRation;
                headData.rightEye2 = (headData.rightEye2+repmat(movePixes,size(headData.rightEye2,1),1)) /scaleRation;
                headData.rightEye3 = (headData.rightEye3+repmat(movePixes,size(headData.rightEye3,1),1)) /scaleRation;
                
                headData.rightEyebrow1 = (headData.rightEyebrow1+repmat(movePixes,size(headData.rightEyebrow1,1),1)) /scaleRation;
                headData.rightEyebrow2 = (headData.rightEyebrow2+repmat(movePixes,size(headData.rightEyebrow2,1),1)) /scaleRation;
                
                headData.noseContour1 = (headData.noseContour1+repmat(movePixes,size(headData.noseContour1,1),1)) /scaleRation;
                headData.noseContour2 = (headData.noseContour2+repmat(movePixes,size(headData.noseContour2,1),1)) /scaleRation;
                headData.noseContour3 = (headData.noseContour3+repmat(movePixes,size(headData.noseContour3,1),1)) /scaleRation;
                headData.noseContour4 = (headData.noseContour4+repmat(movePixes,size(headData.noseContour4,1),1)) /scaleRation;
            ...
            otherwise
                error('viewpoint should be of {''front'',''top'',''back'',''left'',''right''}');
        end
    ...
    if strcmpi(STYLE,'grid')                     % plot grid only
        ...
        Yi = Yi+rmax/g2;
        Yi = Yi(1:end-1);
        Yi = Yi(end:-1:1); % by trial and error!
        %
        ...
        gridvalues(find(gridvalues == cmaplen+1)) = cmaplen; %#ok<*FNDSB>
    ...
    elseif strcmp(STYLE,'contour')                     % plot surface contours only
        ...
        %---- FORCE THE CONTOUR RANGE equal to the colormap range ----/
        if numel(CONTOURNUM) == 1
            validValuesC                      = ZiC(:);
            validValuesC(isnan(validValuesC)) = [];
            
            if isInputContourVals
                
                CONTOURNUM = min(validValuesC):(max(validValuesC) - min(validValuesC) )/CONTOURNUM:max(validValuesC);
            else
                CONTOURNUM = amin:(amax - amin)/CONTOURNUM:amax;
            end
            
            CONTOURNUM(CONTOURNUM==max(validValuesC)) = [];% to remove the dot contour
        end
        
        fprintf('contours   Limits:')
        fprintf(' %-8.4f',CONTOURNUM);
        fprintf('\n');
        %-------------------------------------------------------------\
        ...
        %---- added by yang to control the contour line width ----/
        subh = get(chs, 'children');
        for indsubh = 1:length(subh)
            set(subh(indsubh), 'linewidth', contourLineWidth);
        end;
        %---------------------------------------------------------\
        
        %--- get contour line values ---------/
        contourLineVals = get(chs,'LevelList'); % added by yang to get contour line values
        %-------------------------------------\
    ...
    elseif strcmp(STYLE,'fill')
        ...
        if numel(CONTOURNUM) == 1
            ...
            validValuesC                      = Zi(:); % here we used Zi instead of Zic to make it compatible with line 1277
            validValuesC(isnan(validValuesC)) = [];
            ...
            CONTOURNUM(CONTOURNUM==max(validValuesC)) = [];
        end
        ...
        % in fill mode, the min max should <= and >= the amin and amax
        if numel(CONTOURNUM) > 1
            
            validValuesC                      = Zi(:); % here we used Zi instead of Zic to make it compatible with line 1277
            validValuesC(isnan(validValuesC)) = [];
            
            if (max(CONTOURNUM) < max(validValuesC))||(min(CONTOURNUM) > min(validValuesC))
                CONTOURNUM = [min(validValuesC) CONTOURNUM];
                %     			CONTOURNUM = min(validValuesC):(max(validValuesC) - min(validValuesC) )/numel(CONTOURNUM):max(validValuesC);
                
                cprintf([1 0 0],'In fill mode added min (contourVals) in to the list:\n');
                cprintf([1 0 0],' %-8.4f',CONTOURNUM);
                cprintf('\n');
            end
        end
        %-------------------------------------------------------------\
    ...
    fprintf('Colormap   Limits: min: %-10.4f max: %-10.4f\n',amin,amax);
    % disable by yang 2015/8/13 21:06:25 ----------/
...
if ~strcmpi(STYLE,'grid')                     % if not plot grid only
    ...
    if strcmp(CONVHULL,'on') %%%%%%%%% mask outside the convex hull of the electrodes %%%%%%%%%
        ...
        switch viewpoint
            case  'top'
                ...
                if ~strcmpi(STYLE,'blank')
                    ringh = patch(ringx,ringy,0.01*ones(size(ringx)),BACKCOLOR,'edgecolor','none');
                    hold on;
                end
                % plot(ry*rmax,rx*rmax,'b') % debugging line
        end
    ...
    if headrad > 0                         % if cartoon head to be plotted
        ...
        % viewpoint = 'back';% debug only
        
        if ~ischar(HEADCOLOR) | ~strcmpi(HEADCOLOR,'none')
            
            switch viewpoint
                case 'front'
                    %---------- create the head contours -------------------------------/
                    %---- plot the head contours -----/
                    headContourH1 = plot(headData.headContour1(:,1),headData.headContour1(:,2));
                    headContourH2 = plot(headData.headContour2(:,1),headData.headContour2(:,2));
                    
                    headContourH3 = plot(headData.leftEar1(:,1),headData.leftEar1(:,2));
                    headContourH4 = plot(headData.leftEar2(:,1),headData.leftEar2(:,2));
                    
                    headContourH5 = plot(headData.rightEar1(:,1),headData.rightEar1(:,2));
                    headContourH6 = plot(headData.rightEar2(:,1),headData.rightEar2(:,2));
                    
                    headContourH7 = plot(headData.leftEye1(:,1),headData.leftEye1(:,2));
                    headContourH8 = plot(headData.leftEye2(:,1),headData.leftEye2(:,2));
                    
                    headContourH9 = plot(headData.leftEyebrow1(:,1),headData.leftEyebrow1(:,2));
                    headContourH10 = plot(headData.leftEyebrow2(:,1),headData.leftEyebrow2(:,2));
                    
                    headContourH11 = plot(headData.rightEye1(:,1),headData.rightEye1(:,2));
                    headContourH12 = plot(headData.rightEye2(:,1),headData.rightEye2(:,2));
                    headContourH13 = plot(headData.rightEye3(:,1),headData.rightEye3(:,2));
                    
                    headContourH14 = plot(headData.rightEyebrow1(:,1),headData.rightEyebrow1(:,2));
                    headContourH15 = plot(headData.rightEyebrow2(:,1),headData.rightEyebrow2(:,2));
                    
                    headContourH16 = plot(headData.noseContour1(:,1),headData.noseContour1(:,2));
                    headContourH17 = plot(headData.noseContour2(:,1),headData.noseContour2(:,2));
                    headContourH18 = plot(headData.noseContour3(:,1),headData.noseContour3(:,2));
                    headContourH19 = plot(headData.noseContour4(:,1),headData.noseContour4(:,2));
                    
                    %---------------------------------\
                    
                    %---- control the down side head contour independently ------/
                    if HEADCONTOURDOWNWIDTH == 0
                        set(headContourH2, 'visible','off');
                    else
                        set(headContourH2, 'color',HEADCOLOR,'linewidth', HEADCONTOURDOWNWIDTH);
                    end
                    %------------------------------------------------------------\
                    
                    set(headContourH1, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH3, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH4, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH5, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH6, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH7, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH8, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH9, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH10, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH11, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH12, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH13, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH14, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH15, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH16, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH17, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH18, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH19, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    %-------------------------------------------------------------------\
                    
                case  'back'
                    %---------- create the head contours -------------------------------/
                    %---- plot the head contours -----/
                    headContourH1 = plot(headData.headContourLine1(:,1),headData.headContourLine1(:,2));
                    headContourH2 = plot(headData.headContourLine2(:,1),headData.headContourLine2(:,2));
                    headContourH3 = plot(headData.leftEar1(:,1),headData.leftEar1(:,2));
                    headContourH4 = plot(headData.leftEar2(:,1),headData.leftEar2(:,2));
                    headContourH5 = plot(headData.rightEar1(:,1),headData.rightEar1(:,2));
                    headContourH6 = plot(headData.rightEar2(:,1),headData.rightEar2(:,2));
                ...
                case {'left','right'}
                    ...
                    headContourH1 = plot(headData.headContour1(:,1),headData.headContour1(:,2));
                    headContourH2 = plot(headData.headContour2(:,1),headData.headContour2(:,2));
                    headContourH3 = plot(headData.headContour3(:,1),headData.headContour3(:,2));
                    
                    headContourH4 = plot(headData.headDownLine(:,1),headData.headDownLine(:,2));
                    
                    headContourH5 = plot(headData.eyebrow1(:,1),headData.eyebrow1(:,2));
                    
                    headContourH6 = plot(headData.eyeLine1(:,1),headData.eyeLine1(:,2));
                    headContourH7 = plot(headData.eyeLine2(:,1),headData.eyeLine2(:,2));
                    
                    %---- control the down side head contour independently ------/
                    if HEADCONTOURDOWNWIDTH == 0
                        set(headContourH4, 'visible','off');
                    else
                        set(headContourH4, 'color',HEADCOLOR,'linewidth', HEADCONTOURDOWNWIDTH);
                    end
                    %------------------------------------------------------------\
                    
                    set(headContourH1, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH2, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH3, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH5, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH6, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    set(headContourH7, 'color',HEADCOLOR,'linewidth', HLINEWIDTH);
                    %-------------------------------------------------------------------\
                otherwise
                    error('viewpoint should be of {''front'',''back'',''top'',''left'',''right''}');
            end
    ...
    set(gca, 'xlim', [-xylimValue xylimValue]); set(plotax, 'xlim', [-xylimValue xylimValue]);
    set(gca, 'ylim', [-xylimValue xylimValue]); set(plotax, 'ylim', [-xylimValue xylimValue]);
    ...
    if ~isempty(DIPOLE)
        ...
        if 0 % sum(DIPOLE(1,1:2).^2) > plotrad
            if strcmpi(VERBOSE,'on') %#ok<UNRCH>
...
if endangle-oriangle == 360
    ...
    h2 = patch( [X   + cos(A)*radius(1)], [Y   + sin(A)*radius(end)], zeros(1,segments)+3, colorfill); %#ok<NBRAK>
